...
class LoginSessionTest < ActiveSupport::TestCase
  def setup
    @ls, @code = LoginSession.create_session("test@test.com", "127.0.0.1", "some_user_agent", "example.com")
  end

  test "create_session sends an email" do
    assert_difference("ActionMailer::Base.deliveries.size", 1) do 
      LoginSession.create_session("test@test.com", "127.0.0.1", "some_user_agent", "example.com")
    end 
  end
  ...
  test "active_sessions only contains session that are active" do
    @ls2, @code2 = LoginSession.create_session("test@test.com", "127.0.0.2", "some_other_user_agent", "example.com")
  ...
  test "geoip returns Mountain View for 8.8.8.8" do
    assert LoginSession.geoip("8.8.8.8") == "Mountain View, USA"
  end
end
