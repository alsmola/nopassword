...
Use the :check_valid_session filter in your application controlller:
  before_filter :check_valid_session

If a user is logged in, @current_session will be set the User object.

  @current_session.email # returns the current user email
  @current_session.logout # logs out the user
